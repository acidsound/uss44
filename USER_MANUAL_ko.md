...
### Song 편집
- **패턴 추가**: 상단 패턴 선택기(A-P)에서 원하는 패턴을 탭하면, **현재 선택된 타일(하얀색 테두리) 다음 위치**에 삽입됩니다. 아무것도 선택되지 않았다면 리스트 끝에 추가됩니다.
- **타일 선택 (하얀색 테두리)**: 그리드의 타일을 탭하면 하얀색 테두리가 생기며 선택됩니다. **재생(Play) 시 항상 이 선택된 타일부터 시작**합니다.
- **재생 위치 (빨간색 테두리)**: 현재 재생 중이거나 마지막으로 재생되었던 위치를 나타냅니다. 재생 중 다른 타일을 선택하더라도 음악은 끊기지 않고 원래의 흐름을 유지하므로, 연주 중에 다음 섹션을 미리 준비하기 유용합니다.
