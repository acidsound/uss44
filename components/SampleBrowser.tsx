...
export const SampleBrowser: React.FC<SampleBrowserProps> = ({ onClose, isLandscape }) => {
  ...
  const handleSelectSample = async (url: string, name: string) => {
    ...
    setStatus('Success');
    setTimeout(() => { setStatus(''); onClose(); }, 600);
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      updatePad(targetPadIndex, { sampleId, sampleName: file.name.split('.')[0].toUpperCase(), buffer: audioBuffer });
      await dbService.saveSample({ id: sampleId, name: file.name, data: arrayBuffer });
      setStatus('Success');
      setTimeout(() => { setStatus(''); onClose(); }, 600);
  ...
  const handleSaveRecorded = async (buffer: AudioBuffer, name: string) => {
    ...
    updatePad(targetPadIndex, { sampleId, sampleName: name, buffer });
    const data = buffer.getChannelData(0);
    const arrayBuffer = data.buffer.slice(0);
    await dbService.saveSample({ id: sampleId, name: name, data: arrayBuffer });
