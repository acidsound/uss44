...
export const FileExplorer: React.FC<FileExplorerProps> = ({ mode, type, onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-[90] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in zoom-in-95 duration-200">
      <div className="w-full max-w-2xl bg-[#1e1e22] border border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden max-h-[80vh]">
        ...
        {/* Footer Actions */}
        <div className="p-4 border-t border-white/5 bg-zinc-900/50 flex flex-col gap-3">
           {mode === 'SAVE' && (
              <div className="flex items-center gap-3">
           ...
           <div className="flex gap-3">
              <button 
                onClick={onClose}
                className="flex-1 py-3 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-400 font-bold uppercase text-xs transition-colors"
              >
                Cancel
              </button>
              <button 
                ...
                className="flex-[2] py-3 rounded-xl bg-retro-accent hover:bg-retro-highlight text-white font-bold uppercase text-xs transition-all shadow-lg active:scale-[0.99] disabled:opacity-50 disabled:pointer-events-none flex items-center justify-center gap-2"
              >
