...
### Improved
- **Song Mode UX Refinement**: 
    - **독립적 테두리 관리**: 재생 위치(빨간색)와 사용자 선택(하얀색) 영역을 분리하여 편집 편의성 증대.
    - **비간섭 편집**: 재생 중 패턴을 추가하거나 위치를 바꿔도 음악이 끊기거나 점프하지 않도록 개선.
    - **재생 시작 지점 고정**: 재생 버튼 클릭 시 항상 하얀색 테두리(선택) 위치부터 연주 시작.
    - **인덱스 자동 보정**: 패턴 삽입/삭제 시 현재 재생 중인 음악적 위치를 유지하도록 인덱스 자동 추적.
